...
def pruebas():
    ...
    with open("C:/Users/Sebastian/APP_OCR_APP/imageprueba.png", "rb") as fd:
        ...
        poller = document_analysis_client.begin_analyze_document("prebuilt-document", invoice)
            ...
            for span in style.spans:
                ...
                if Estado>-1:
                    if len(diccionario["Estado_Animo"]) == 0:    
                        subcadena = cadena[Estado+17:span.offset+ span.length]
                        diccionario["Estado_Animo"] = subcadena
                if Orina>-1:
                    if len(diccionario["Orina"]) == 0:    
                        subcadena = cadena[Orina+2:span.offset+ span.length]
...
if __name__ == '__main__':
    ...
    # scheduler.start()
    app.run(debug=True, port=8000, host='192.168.17.18')
